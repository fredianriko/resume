...
export default function HeroSection() {
  return (
    <section
      id="hero"
      className="relative flex min-h-screen items-center overflow-hidden bg-gradient-to-b from-slate-50 to-white px-6 dark:from-slate-950 dark:to-slate-900"
    >
      {/* Decorative glows */}
      <div className="pointer-events-none absolute -top-32 -right-24 h-96 w-96 rounded-full bg-blue-500/20 blur-3xl dark:bg-blue-600/20" />
      ...
      <div className="relative mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 py-28 md:grid-cols-2 md:py-0">
        {/* Left: intro */}
        <div className="order-last text-center md:order-first md:text-left">
          ...
          <h1 className="bg-gradient-to-r from-slate-900 to-slate-600 bg-clip-text text-5xl font-bold tracking-tight text-transparent md:text-6xl dark:from-white dark:to-slate-400">
            {hero.name}
          </h1>

          <div className="mt-4 flex items-center justify-center gap-2 text-2xl font-semibold text-slate-700 md:justify-start md:text-3xl dark:text-slate-200">
            <span className="text-blue-600 dark:text-blue-400">▸</span>
            <RotatingText items={ROLES} />
          </div>

          <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-slate-600 md:mx-0 md:text-lg dark:text-slate-300">
            {hero.subtitle}
          </p>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4 md:justify-start">
            ...
            {contact.resumeUrl && (
              <a
                ...
                className="rounded-lg border border-slate-300 px-6 py-3 text-base font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
              >
                Download Resume
              </a>
            )}
        ...
        <div className="order-first flex justify-center md:order-last md:justify-end">
          <div className="relative">
            {/* gradient ring */}
            <div className="absolute -inset-3 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-400 opacity-30 blur-md" />
            <div className="relative rounded-full bg-gradient-to-tr from-blue-500 to-indigo-400 p-1.5">
              <img
                src={asset(hero.profileImage)}
                alt={hero.name}
                className="h-56 w-56 rounded-full border-4 border-white object-cover shadow-2xl md:h-80 md:w-80 dark:border-slate-900"
              />
            </div>
          </div>
        </div>
...
function SocialLink({
  ...
  return (
    <a
      ...
      rel="noopener noreferrer"
      aria-label={label}
      className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-slate-300 text-slate-600 transition hover:border-blue-500 hover:text-blue-600 dark:border-slate-700 dark:text-slate-300 dark:hover:border-blue-500 dark:hover:text-blue-400"
    >
      {children}
    </a>
  );
